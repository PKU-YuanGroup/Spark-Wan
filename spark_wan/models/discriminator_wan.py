...
class WanDiscriminator(WanTransformer3DModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if head_type == "complex":
            ...
            #     nn.SiLU(True),
            #     nn.Dropout(cnn_dropout),
            #     nn.Conv3d(256, 128, kernel_size=(4, 4, 4), stride=2, padding=1),
            #     nn.SiLU(True),
            #     nn.Dropout(cnn_dropout),
            #     nn.Conv3d(128, 1, kernel_size=(4, 4, 4), stride=2, padding=1),
            # )
    ...
    def forward(
        ...
        if self.config.head_type == "seaweed":
        ...
        elif self.config.head_type == "psdual":
            ...
            perceptual_score = self.perceptual_head(hidden_states).mean(dim=(1,2,3,4))
            ...
            output = torch.cat(output_list, dim=1)
            output = self.semantic_head(output)
            if sequence_parallel_group:
                output = Gather.apply(sequence_parallel_group, output, 1)
            
            semantic_score = output.mean(dim=(1, 2))
            output = 0.2 * perceptual_score + semantic_score
        else:
            # GAN Distill Need
            if sequence_parallel_group:
                hidden_states = Gather.apply(sequence_parallel_group, hidden_states, 1)
