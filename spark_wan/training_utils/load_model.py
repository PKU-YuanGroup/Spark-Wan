...
def load_model(
    ...
    if enable_discriminator:
        ...
        if model_config.is_train_disc_lora:
            ...
            if hasattr(discriminator, "disc_head"):
                discriminator.disc_head.requires_grad_(True)
            elif hasattr(discriminator, "semantic_head"):
                discriminator.semantic_head.requires_grad_(True)
                discriminator.perceptual_head.requires_grad_(True)
            else:
                raise ValueError("No disc_head or semantic_head found in discriminator")
