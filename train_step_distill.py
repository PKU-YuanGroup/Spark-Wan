...
from spark_wan.training_utils.gan_utils import (
...
from spark_wan.models.ema_model import EMA
...
def main(args: Args):
    ...
    if args.model_config.enable_ema:
        ema_transformer = EMA(
            transformer,
            decay=args.model_config.ema_decay,
            fsdp_resharded="transformer" in args.model_config.reshard_after_forward,
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                if is_generator_step:
                    ...
                    unwrap_model(discriminator).requires_grad_(requires_grad=True)
            ...
            if is_generator_step and args.model_config.enable_ema:
                if ema_transformer.is_registered:
                    ema_transformer.update()
                elif global_step >= args.model_config.ema_start_step:
                    ema_transformer.register()
            ...
            if real_global_step % args.training_config.checkpointing_steps == 0:
                ...
                if args.model_config.enable_ema and ema_transformer.is_registered:
                    ema_transformer.save_ckpt(
                        os.path.join(checkpoint_path, "ema_model.safetensors")
                    )
            ...
            if (
                ...
                for validation_prompt in validation_prompts:
                    ...
                    with torch.no_grad() and torch.amp.autocast(
                        ...
                        if args.model_config.enable_ema and ema_transformer.is_registered:
                            ...
                            log_validation(
                                ...
                                pipeline_args=pipeline_args,
                                global_step=real_global_step,
                                phase_name="student/validation_ema",
                                global_rank=global_rank,
                            )
                    ...
                    if (
                        real_global_step == 1
                        and args.validation_config.log_teacher_sample
                    ):
                        unwrap_model(transformer).disable_adapter_layers()
